...
const UPLOAD_DIR = path.join(resolveProjectRoot(), "data", "production-os", "uploads");
...
function buildKpiNote(formData: FormData) {
  const views = parseNumber(String(formData.get("kpiViews") || ""));
  const ctr = parseNumber(String(formData.get("kpiCtr") || ""));
  const watchTime = parseNumber(String(formData.get("kpiWatchTime") || ""));
  const memo = String(formData.get("kpiMemo") || "").trim();

  if (views === null && ctr === null && watchTime === null && !memo) {
    return null;
  }
...
export async function createItem(formData: FormData) {
  ...
  const kpiNote = buildKpiNote(formData);
...
export async function rerunJobRun(formData: FormData) {
  ...
  revalidatePath(`/items/${previous.itemId}`);
  revalidatePath("/");
}
